...
<div class="aside aside-left aside-fixed d-flex flex-column flex-row-auto" id="kt_aside">
    ...
    <div class="brand flex-column-auto" id="kt_brand">
        ...
        <a href="{{ route('home') }}" class="brand-logo">
            <img alt="Logo" style="width: 50px" src="{{asset(\App\Models\Setting::getSingleRow('logo'))}}" />
        </a>
        <!--end::Logo-->
        <!--begin::Toggle-->
    ...
    <div class="aside-menu-wrapper flex-column-fluid" id="kt_aside_menu_wrapper">
        ...
        <div id="kt_aside_menu" class="aside-menu my-4" data-menu-vertical="1" data-menu-scroll="1" data-menu-dropdown-timeout="500">
            ...
            <ul class="menu-nav">
                ...
                @endcan
                @can('show_products')
                    <x-admin.menu-item href="{{route('admin.product')}}" icon="flaticon2-box-1"  :active="request()->routeIs(['admin.product','admin.store.product'])" label="محصولات" />
                @endcan
                @can('show_articles')
                ...
                @endcan
                @can('show_orders')
                    <x-admin.menu-item href="{{route('admin.order')}}" icon="flaticon2-box"  :active="request()->routeIs(['admin.order','admin.store.order'])" label="سفارش ها ({{\App\Models\Order::getNew()}})" />
                @endcan
                @can('show_orders')
                    <x-admin.menu-item href="{{route('admin.refund')}}" icon="flaticon2-refresh-button"  :active="request()->routeIs(['admin.refund','admin.store.refund'])" label="مرجوعی ها ({{\App\Models\Refund::getNew()}})" />
                @endcan
                @can('show_comments')
                ...
                @endcan
                @can('show_reductions')
                    <x-admin.menu-item href="{{route('admin.reduction')}}" icon="flaticon-price-tag" :active="request()->routeIs(['admin.reduction','admin.store.reduction'])" label="کد های تخفیف" />
                @endcan
                @can('show_settings')
                    <x-admin.menu-group icon="flaticon2-settings" :active="request()->routeIs(
                    ['admin.setting.base','admin.setting.home','admin.setting.aboutUs','admin.setting.contactUs','admin.setting.faq','admin.store.faq'])" label="تنظیمات" >
                        ...
                        @endcan
                            @can('show_settings_fag')
                                <x-admin.menu-item href="{{route('admin.setting.faq')}}" icon="menu-bullet menu-bullet-dot" :active="request()->routeIs(['admin.setting.faq','admin.store.faq'])" label="سوالات متداول" />
                            @endcan
                    </x-admin.menu-group>
                @endcan
