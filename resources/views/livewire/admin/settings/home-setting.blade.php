    ...
    <div class="card card-custom gutter-b example example-compact">
        ...
        <div class="card-body">
            <x-admin.form-section label="اسلایدر ابتدا">
                <x-admin.forms.validation-errors/>
                <x-admin.forms.input type="text" id="slider" label="عنوان*" wire:model.defer="slider"/>
                <x-admin.forms.input type="text" id="subtitle" label="زیر عنوان" wire:model.defer="subtitle"/>
                <x-admin.forms.lfm-standalone id="sliderImage" label="تصویر پسزمینه*" :file="$sliderImage" type="image" required="true" wire:model="sliderImage"/>
                <x-admin.forms.input type="url" id="sliderLink" label="لینک مورد نظر" wire:model.defer="sliderLink"/>


                <x-admin.forms.full-text-editor id="homeAbout" label="ما چه کسی هستیم*" wire:model.defer="homeAbout"/>

                <x-admin.forms.lfm-standalone id="homeImg1" label="تصویر1" :file="$homeImg1" type="image" required="true" wire:model="homeImg1"/>
                <x-admin.forms.lfm-standalone id="homeImg2" label="تصویر2" :file="$homeImg2" type="image" required="true" wire:model="homeImg2"/>
                <x-admin.forms.lfm-standalone id="homeImg3" label="تصویر3" :file="$homeImg3" type="image" required="true" wire:model="homeImg3"/>
...
{{--            </x-admin.form-section>--}}
            <x-admin.modal-page id="content" title="{{ $titleContent }}" wire:click="storeContent">
                ...
                @if($category <> 'banners')
                    ...
                    @if($type == 'slider')
                        <x-admin.forms.dropdown id="row" :data="$data['row']" label="تعداد ردیف *" wire:model.defer="row"/>
                    @endif
                    @if($type <> 'slider')
                    ...
                    <x-admin.button class="primary" content="افزودن مورد" wire:click="addContentCase('new')" />
                    @foreach($contentCase as $key => $value)
                        <div style="display: flex;align-items: center">
                            <x-admin.forms.input type="text" id="contentCase{{$key}}" label="شماره شناسه*" wire:model.defer="contentCase.{{$key}}"/>
